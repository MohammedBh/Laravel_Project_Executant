            ...
            <section class="p-6 mx-auto grid grid-cols-4 gap-4">
                @foreach ($users as $user)
                    <div class="max-w-xs mx-auto overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800">
                        <img class="object-cover w-full h-56 px-8" src="{{ asset('storage/img/' . $user->avatars->src) }}"
                            alt="avatar">

                        <div class="py-5 text-center">
                            <span
                                class="block text-2xl font-bold text-gray-800 dark:text-white px-2">{{ $user->name . ' ' . $user->lastname }}</span>
                            <span class="text-sm text-gray-700 dark:text-gray-200">{{ $user->email }}</span>
                            <div>
                                <span class="text-sm text-gray-700 dark:text-gray-200">{{ $user->age }}y | <span
                                        class="  font-bold @if ($user->role_id == 1)
                                    text-red-500
                                    @else
                                    text-green-500
                            ...
                            <div class="text-center flex justify-around mt-3">
                                ...
                                <a href="/users/{{ $user->id }}/edit"
                                    class="px-4 py-1 text-xs font-bold text-white uppercase transition-colors duration-200 transform bg-green-500 rounded hover:bg-green-400"><svg
                                        xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
